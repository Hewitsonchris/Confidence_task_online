...
const MOVE_SCALE = 0.25 // factor to combat pointer acceleration
const PI = Math.PI
const MAX_RT = 1000
const MAX_MT = 300
...
export default class MainScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.origin = new Phaser.Geom.Circle(0, 0, CENTER_SIZE_RADIUS)
      ...
      //move
      this.move_instruct = this.add.text(0, 450, 'Move!', {
        fontFamily: 'Verdana',
        color: '#ffffff',
        backgroundColor: '#000000',
        fontSize: 40,
      ...
      if (this.input.mouse.locked) {
        ...
        for (let evt of evts) {
          ...
          this.extent = extent
            ...
            //if (this.confidence_thumb.visible) { // ADDED CODE FOR CONDIDENCE RATING: REQUIRE NEW IF THIS.STATE?
            //  this.confidence_thumb.x = clamp(this.raw_x, -300, 300) // So the user cannot leave the bar
           // }
          }
        }
      }
...
Once you've rated your confidence, right click to register your rating, then reach again!`
    ...
    instruct_txts['instruct_clamp'] =
      ...
      instruct_txts['instruct_wash'] =
        'Great work! You are almost finished! In this section you will not need to rate confidence. Also, you wont see the cursor when reaching. Still reach straight to the target, even though you will not see the cursor.'
  ...
  update() {
    ...
    case states.INSTRUCT:
      ...
      case states.REPORT:
        if (this.entering) {
            this.keypress =false
            let grad_bool = this.current_trial.grad_bool
            let sign_val = this.current_trial.sign_val
            let grp = this.current_trial.group_num
            let max_per = this.current_trial.max
            console.log("group: " + grp)
            console.log("manipulation: " + this.current_trial.manip_angle)
            console.log("step: " + this.current_trial.step_val)
            console.log("max: " + max_per)
            console.log("sign: " + sign_val)
            console.log("gradual?: " + grad_bool)
            this.entering = false
            this.user_cursor.visible = false
            this.fake_cursor.visible = false
            this.aim_line.visible = this.current_trial.aim
            this.aim_reticle.visible = this.current_trial.aim
            this.aim_txt.visible = this.current_trial.aimkeyboard
            var rad = Phaser.Math.DegToRad(this.current_trial.target_angle)
            this.target.x = TARGET_DISTANCE * Math.cos(rad)
            this.target.y = TARGET_DISTANCE * Math.sin(rad)
            this.target.fillColor = GREEN
            ...
            //THIS DOES NOT WORK. NOT SURE WHERE TO ADD CONFIDENCE ELEMENTS
            this.confidence_thumb.y = 0
            this.confidence_scale.setFillStyle(0xffff00)
            this.confidence_value = 50
            this.confidence_txt1.setVisible(true)
            this.confidence_txt2.setVisible(true)
            this.confidence_instruct.setVisible(true)
            this.confidence_scale.setVisible(true)
            this.confidence_thumb.setVisible(true)
         ...
         //   this.input.keyboard.once('keydown-ENTER', () => {
                ...
                this.confidence_txt1.visible = false
                this.confidence_txt2.visible = false
                this.confidence_instruct.visible = false
                this.confidence_thumb.visible = false
                this.confidence_scale.visible = false
                this.confidence_value = Math.round(((-this.confidence_thumb.y + 50) / 100) * 100)
  ...
  break
    ...
    case states.PRETRIAL:
      if (this.entering) {
          ...
          this.move_instruct.setVisible(true)
        ...
        //set up the cursor cloud at this point
          if (this.current_trial.cursor_cloud_sd > 0) {
              ...
              for (var i = 0; i < n_dots; i++) {
                  dots[i] = this.add.circle(xs[i], ys[i], DOTS_SIZE_RADIUS, LIGHTBLUE, 0.2) //change dot radius
              }
    ...
    case states.POSTTRIAL:
      if (this.entering) {
        this.entering = false
        this.move_instruct.setVisible(false)
        ...
        combo_data['confidence'] = this.confidence_value
  ...
  next_trial() {
    ...
    if (this.tmp_counter > this.total_len) {
      ...
      this.progress.text = `${'Trial: ' + this.tmp_counter} / ${this.total_len}`
    }
    ...
    if (cur_trial === undefined || this.trials.length < 1 && tt.startsWith('break')) {
      ...
      tt.startsWith('practice') ||
        ...
        tt.startsWith('wash') || tt.startsWith('adapt')
    ) {
        if (this.current_trial.aim) {
            this.state = states.AIM
            this.target.visible = true
        } else if (this.current_trial.report) {
            this.state = states.REPORT
            this.target.visible = true
        } else {  this.state = states.PRETRIAL
